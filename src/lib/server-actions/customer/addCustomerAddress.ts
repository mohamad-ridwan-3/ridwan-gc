...
import { ReqAddCustomerAddressT, ResultAddCustomerAddressT } from "@/src/types/profile";
import { revalidatePath } from "next/cache";
import { redirect } from "next/navigation";

export default async function addCustomerAddress(reqData: ReqAddCustomerAddressT): Promise<ResultAddCustomerAddressT> {
    const cookieStore = cookies()
    const auth_session = cookieStore.get(session_login_name)
    const baseURL: string = await getBaseURL()
    if (auth_session?.value) {
        const response = await fetch(`${baseURL}/api/store/address/add`, {
            cache: 'no-store',
            method: 'POST',
            headers: {
